...
import {IP_ADDRESS, TransferData} from '../../models/types'


// API call to transfer PRX tokens
const transferTokens = async (data: TransferData): Promise<boolean> => {
  const url = `http://${IP_ADDRESS}:3000/transaction/transfer/prx` 
...
const transferUSDT = async (data: TransferData): Promise<boolean> => {
    const url = `http://${IP_ADDRESS}:3000/transaction/transfer/usdt` 
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
